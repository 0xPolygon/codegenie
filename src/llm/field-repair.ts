...
import { cleanupSubmitShape, focusedRepairDiagnostics, submissionIssues } from "./submit-preservation.js";
...
function updateSchema(input: Schema, baseline: unknown): Schema {
  ...
  if (schema.type === "object" && object(baseline)) {
    ...
    if (schema.properties) schema.properties = Object.fromEntries(Object.entries(schema.properties).map(([key, child]) => {
      const updated = updateSchema(child, baseline[key]);
      updated.description = `${child.description ?? ""} ${required.has(key) ? "Required in the final merged result; omit from this update only if already retained." : "Optional in the final result and in this update."}`.trim();
      return [key, updated];
    }));
  } else if (schema.type === "array" && schema.items) {
    if (Array.isArray(baseline) && baseline.length && baseline.some(object)) {
      const items = schema.items;
      schema.items = baseline.map((value, index) => updateSchema((Array.isArray(items) ? items[index] : items) ?? {} as Schema, value));
...
export function createFieldRepair(schema: TSchema, original: unknown, allowSemanticRepair = false, replacementGroups: readonly (readonly string[])[] = []): FieldRepair | undefined {
  ...
  const issues = submissionIssues(schema, baseline);
  ...
  const partial = updateSchema(schema as Schema, baseline);
  ...
  return {
    ...
    paths,
    baseline,
    diagnostics: focusedRepairDiagnostics(schema, original),
    merge(values) {
