  ...
  return {
    ...
    buildPacketReviewPrompt: ({ packet, skills }) => {
      ...
      return buildPrompt(7, [
        ...
        "If the packet includes reviewQuestions, treat them as obligations and answer only those exact question IDs. If no questions are attached, omit answeredQuestions. role:\"primary\" means this packet owns the full answer, candidate, or exact unresolved predicate. role:\"supporting\" means answer only the local slice visible here, cite local evidence, and mark partial when the full proof belongs to the primary packet or Stage 8. Unowned questions use the normal full-answer behavior.",
