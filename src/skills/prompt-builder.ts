  ...
  return {
    ...
    buildVerifierPrompt: ({ candidate, originContext, hunksText, intentSignals, skills }) => {
      ...
      return buildPrompt(9, [
        ...
        "Assess the defect independently from its proposed fix and regression test. When endorsing a suggestion, inspect the relevant caller/spec with the existing tools and budget: does the remedy preserve the caller-visible invariant, and does the test exercise the failing boundary and require the intended outcome rather than the buggy behavior? Internal consistency alone is insufficient. For example, lowering a promised minimum is not a remedy if callers require at least the original request.",
        "Optionally provide suggestionAssessments.suggestedFix and/or suggestedTest with status supported, incompatible, or unverified, rationale, source evidence locations, and suggestionText exactly matching the final suggestion after findingUpdates. Assess fix and test independently. Omission means unverified; absent suggestions need no assessment. Missing remedy proof must not invalidate an established defect. If budget/evidence is insufficient, mark unverified or omit the assessment, and do not invent support. An incompatible suggestion should be revised if a supported replacement is known; otherwise retain it only as an unendorsed historical proposal.",
        "Provide proofAssessment with status established, refuted, or unresolved; cite concrete source evidence and list unresolved assumptions with essential=true only when the defect itself depends on them. A changed comment or contradictory old/new assumptions is not proof that the new code is wrong. Keep/revise requires established proof and no unresolved essential assumption. Return reject with status unresolved when proof is missing; the harness retains those questions for human attention. Uncertainty limited to severity or secondary reach can coexist with an established defect. When source observations disagree, resolve them from inspected code or retain the disagreement explicitly; do not present incompatible statements as settled facts.",
