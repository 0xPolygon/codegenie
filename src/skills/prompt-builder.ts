  ...
  return {
    ...
    buildComposerPrompt: ({ groupedFindingsJson, intent, coverage, followUpHintNotes }) => {
      ...
      return buildPrompt(10, [
        ...
        "For every composed finding, supply sections [{kind: impact|verification|fix|test, text, sourceRefs}] and evidenceRefs, using sourceComponents IDs from the grouped findings. Only use source IDs actually supplied: optional absent fields have no source ID. Every component of every merged finding must be referenced exactly once, in its matching kind; evidence components go only in evidenceRefs. Combine paraphrases into one section while retaining unique conditions and uncertainty. Do not copy all member prose. The harness renders evidence and verification/caveats verbatim from the referenced inputs. finalBody is a short legacy summary. Do not repeat member severity judgments in sections; preserve real caveats and disagreements. Keep independently actionable testing issues separate from production defects.",
