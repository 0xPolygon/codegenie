...
- [ ] Phase 2: Git layer and change inventory — `GitClient` subprocess chokepoint, input resolver for branch/commit modes plus bare default (PR mode deferred to Phase 6), diff parser (hunk ids, line mapping, statuses, anchors index), shared detector library + Stage 2 detect/filter pass + Stage 3 classification of kept files. → `components/repository_and_github.md`
