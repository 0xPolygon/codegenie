...
6. Implement `src/evals/eval-command.ts` and wire `codeninja eval` into `src/cli/main.ts`, including `--eval-dir`, `--from-artifacts`, `--cache`, `--no-cache`, stdout summaries, and exit code mapping.
