...
1. Pin runtime dependencies in `package.json`/`pnpm-lock.yaml`: `web-tree-sitter`, `tree-sitter-go`, `tree-sitter-typescript`, `tree-sitter-javascript`, `@vscode/ripgrep`, and `p-limit`. Verify `web-tree-sitter` exposes parse interruption via `ParseOptions.progressCallback`, and verify the installed tarballs include `tree-sitter.wasm`, `tree-sitter-go.wasm`, `tree-sitter-typescript.wasm`, `tree-sitter-tsx.wasm`, and `tree-sitter-javascript.wasm`.
...
4. Add `src/repo/source-resolver.ts` to derive `RevisionBinding { headCommit, baseCommit }` from `ResolvedReviewInput`, resolve `SourceSelector`, memoize plumbing reads/blob shas, list files, and capture the `WorktreeSnapshot` used by search-engine selection.
...
7. Add `src/repo/search.ts`, `src/repo/diff-blocks.ts`, and `src/repo/likely-tests.ts` implementing `searchFiles`, `findSymbolMentions`, `findDefinition`, `readDiffBlocks`, `findLikelyTests`, and `listFiles` with result caps, revision reads, best-effort ripgrep fast path, context enrichment, and meta/degradation fields.
...
- `search tools use git/ripgrep semantics`: verifies `searchFiles`, context windows, symbol enrichment, mention verification, base-side search, caps, and telemetry engine fields.
