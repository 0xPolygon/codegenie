...
Status: COMPLETE
Completed: 2026-07-02. Default decision (superseding the draft's $35≈7.5M framing): sized by rule — **~25% above the largest observed full review, rounded** (`0c4d5213/42`: 4,676,284 tokens; ×1.25 = 5.85M, rounded up to 6M per owner decision 2026-07-02) → `maxBudgetTokens: 6_000_000` (~$28 at the observed opus blend). Headroom note: with the 15% reserved tail, dispatch soft-stops at 5.1M — ~9% above observed full-review usage. **Watch item: plan 84's ensemble adds packet-review tokens; expect to raise the default (or lean on `--budget-boost`) when it lands, and re-derive from the then-largest observed run.** Rename was a clean break (`maxTotalTokens`/`max_total_tokens` fully removed from src/tests/specs); eval cases can override via `review.maxBudgetTokens`; the value is always visible in `budget-summary.json` configured/effective and `info.json` effectiveConfig.
