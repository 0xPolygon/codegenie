...
Status: COMPLETE (2026-07-04) — output-quality slice is the final form; deletion steps CLOSED not-executed per the 2026-07-04 decision record (promotion converted zero-native in run 51 where the adaptive pass whiffed 3 straight; head-to-head evidence split)
Implementation notes (2026-07-02, per the corrected priors above — promotion is load-bearing, so output quality shipped first and deletions wait for repeat data): (1) **No fabricated anchors** — `promotedCandidate` emits `anchor: undefined`, `changedLine: false`, `modelAnchorSubmitted: false`; plan 76's gate-only representative anchor proves on-diff-ness at verification and only a verifier-revised anchor can publish inline (the run-24/30 first-changed-line lie is structurally impossible now). (2) **No template titles** — the title is the hint's own predicate text (bounded 140 chars); failureMode leads with the predicate instead of "The review raised a concrete unresolved behavior predicate:" boilerplate. (3) **Evidence scoped** to the hunk(s) the predicate's symbols reference (first hunk as fallback) — never the whole packet dump. (4) Category-from-lens reorder was attempted and REVERTED: the existing fixture "routes a test-scoped hint with a runtime predicate as correctness" pins the opposite (a runtime predicate in the reason wins over test scope), and no text-level rule separates it from the run-42 shape — the run-42 category fix needs the hint schema to carry an explicit category (deferred, noted for the Stage-7 schema work in plan 84's lane). Admission predicate NOT rewritten: the existing eligibility gates already require concrete file/symbol + changed scope + concrete failure predicate + evidence (the fable three-part rule, more granular), and the corrected priors argue against tightening before repeat data. Classifier/locality/reserve deletions and the compensation-lane shrink (steps 4-5) remain gated on Plan-79 repeat measurements.
...
Evidence update (2026-07-04, run `0c4d5213/53` — first head-to-head): the same near-miss signal on the same packet was answered by BOTH mechanisms — the promotion lane's synthetic candidate (`a81d5adf-u3`) was **rejected** at verification while plan 92's adaptive second pass produced a real candidate (`a81d5adf-a2f1`) that was **kept and matched the expectation**. First direct evidence that a genuine second draw outperforms a fabricated candidate on the identical signal; strengthens this plan's gated-deletion premise, still decided on accumulated rates per the stop conditions.
