...
Status: COMPLETE
Completed: 2026-07-02. Implementation notes: the soft/hard pair lives entirely in `pi-runner` keyed off `request.timeoutMs` (which packet and verifier tasks already pass), so no worker-task deadline contract was needed — the soft deadline reuses the existing `queueForcedFinalizePrompt` mechanism with a new `soft_deadline` reason, and `worker-runner`'s timer became the hard backstop at `timeoutMs + finalizeGraceMs`. Grace formula adjusted from the draft to stay proportional for small (test-sized) budgets: `min(240s, max(t/4, min(120s, t)))`. Incomplete verifications now carry first-class `verdict: "incomplete"` (runner-assigned only; the model submit schema still excludes it) with truthful `errorCode` (`worker_timed_out`/`budget_limited`/`not_dispatched`). Timed-out passes are excluded from the plan-80 re-dispatch. Deferred: aggregate grace counters in budget-summary (events `full_finalize_started{reason:soft_deadline}` and `finalize_grace_used` cover triage).
