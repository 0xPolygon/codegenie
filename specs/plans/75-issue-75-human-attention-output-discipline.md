...
Status: COMPLETE (step 1, 2026-07-04; step 2 superseded per header)
Step-1 implementation notes: VerificationResolution gains a source mode; a completed reject with falsePositiveRisk=high on a PROMOTED candidate ("stage9_adjudicated_reject") resolves its exact source note via provenance-only matching (no fuzzy fallthrough — enforced structurally in attentionGroupResolvedByVerification); suppression records read "adjudicated by stage 9 reject verdict for <id>"; stage-10 event human_attention_hints_suppressed_by_adjudicated_reject. Uncertain rejects (medium/low risk or incomplete) leave notes visible. Pinning tests landed 2026-07-04 (`tests/human-attention-adjudication.test.ts`): confident reject suppresses the exact source note with provenanceMatched=true; uncertain (medium-risk) reject produces no resolution and the note survives; adjudicated rejects never fuzzy-match unrelated notes.
