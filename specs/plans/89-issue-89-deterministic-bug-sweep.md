...
Status: COMPLETE — phase A complete (2026-07-02); phase B complete (2026-07-04).
...
Phase B implementation notes: B1-B9 landed as eval-independent hygiene: tree-sitter eviction disposes trees and hashes raw-content cache keys; telemetry lifecycle endpoints are idempotent; crashed run dirs prune by mtime; existing `.codegenie/.gitignore` files are create-if-absent only; Go value receivers render `(T).Method`; HTTP status mining is anchored to status/code/HTTP contexts; extra submit calls emit `extra_submit_dropped`; stale PR-ref lock removal re-checks before takeover; symbol suffix matching requires a segment boundary; related-context omissions use `hunkIds`; `readRange` clamps past EOF; eval run ids are no longer the artifact directory name; README spec paths are corrected; old-layout `--from-artifacts` replay errors explicitly with `old layout unsupported`.
