...
- [ ] **Provider serialization follow-up (diagnostic, no code)** — MAJOR UPDATE (runs 46/47, started 45s apart): two concurrent run processes each sustained full per-run throughput (~1,360 tok/s each; ~2,670 aggregate vs 1,455 for solo run 45) while per-call TTFB stayed queued (stage-7 p50 61-63s) — the serialization is **per-process, not per-account** (healthy-day solo reference: 6,218 tok/s, run 42). Next: cheap decisive probe — N concurrent minimal calls in ONE process vs N single-call processes; if per-process client-side (undici/SDK connection pooling), fix locally in the adapter; else Anthropic ticket with captured request-ids.
- [ ] **maxBudgetTokens re-derivation (plan-90 watch item, triggered)** — run 46 set a new largest observed full review (4,925,828 tokens), within 3.4% of the 5.1M dispatch soft-stop; the sizing rule (25% above largest observed) now yields 6.16M > the 6M default. Recommend raising to 6.5M (or 7M ahead of plan 84's ensemble). Owner decision on the number.
...
- [ ] **pi-ai Models-API migration (needs plan)** — 0.80.3 moved the legacy global API to a temporary `/compat` entrypoint (adopted 2026-07-02, verbatim behavior, scheduled for upstream removal). The real migration (`createModels()` + provider factories, `models.getAuth()` replacing env-key injection, credential-store contract, `fauxProvider()` in tests) is adapter surgery on the same seam as Plan 86 step 3 — bundle them in one window, validate on the harness. Pin pi-ai to 0.80.x until done.
- [ ] **Plan 81** — minimal predicate-only uncertainty promotion (fable D2). CORRECTED PRIOR (2026-07-02, runs 24-30): promotions rescued the published finding in every stage-7-silent run (24, 28, 30) — load-bearing for recall on 49f4645b, not pure chaff (~4 kept of ~13). Admission rule stands (rescues carry concrete predicates; "Verify X" chaff doesn't); deletion lean contra-indicated pending repeat data; output-quality fixes (real titles, no fabricated anchors) now the highest-value slice.
