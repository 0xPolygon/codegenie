...
- [ ] **Baseline runs (owner-run)** — the repeat-10 campaign was replaced by owner-run manual evals (2026-07-02 decision). Status: `49f4645b` ×2 DONE (runs 29-30, both pass, API key — one direct-lane, one promotion-rescue); `0c4d5213` ×2 pending. Caveat recorded: 2 runs/case gives weak rates — Plan 84's before/after validation will still need `repeat: N` runs on the relay case when it lands; the harness is ready.
- [ ] **Provider serialization follow-up (diagnostic, no code)** — server-side ~1-request-at-a-time queueing persists across auth lanes (OAuth + API key), pi-ai versions, and days (runs 26-30: solo TTFB 1-2s; 1-3 in-flight TTFB 50-90s). Not session-key-caused (keys inert on direct Anthropic; per-worker keys landed and pattern unchanged). Next: (a) run the two-simultaneous-cases discriminator during the 0c4d5213 baseline runs (per-account vs narrower), (b) if it persists, Anthropic support ticket — per-call request-ids are captured in model-calls.jsonl.
