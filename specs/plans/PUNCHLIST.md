...
- [ ] **Plan 91** — pi-ai Models-API migration off `/compat` code implemented 2026-07-03; owner A/B protocol-posture diff remains pending before measurement-campaign use. Auth decision: Option A, runtime calls use codegenie-owned env → stored api_key → stored OAuth-with-refresh resolution with per-call apiKey injection. Pi-ai is unpinned to `^0.80.3` (latest is still 0.80.3).
