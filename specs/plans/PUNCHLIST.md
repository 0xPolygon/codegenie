...
- [ ] **Per-worker session IDs (small, land before 86 step 5)** — stage-scoped session IDs map to OpenAI's `prompt_cache_key`, concentrating all Stage-7/9 workers on one cache node above OpenAI's documented ~15 req/min per-key overflow threshold; per-worker IDs (`stage-7-<workerId>`) fix gpt-5.5 cache routing, are inert on direct Anthropic, and avoid gateway affinity pinning. See the provider matrix note in skills_llm_telemetry.md (2026-07-02).
