...
- [ ] **Plan 91** — pi-ai Models-API migration off `/compat` (planned 2026-07-03). Five phases (catalog reads → auth reconciliation → call path → tests → flip+unpin), each landable alone; protocol contract enumerated (tool-choice dialects incl. 86.3 forcing, reasoning mapping, auth order, onResponse/sessionId/maxRetries, catalog semantics); gate = A/B protocol-posture diff on the harness. Land in a quiet window between measurement campaigns, never mid-campaign. Pi-ai stays pinned to 0.80.x until phase 5.
