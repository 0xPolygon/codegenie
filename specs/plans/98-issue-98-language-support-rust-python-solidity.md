...
Status: COMPLETE — Phases 0-6 implemented and validated; this atomic Phase-6 commit is the boundary revision, with external visibility beginning only on its first push or merge
...
Post-completion corrective extension: completed Plan 99 (`99-issue-99-plan-98-correctness-follow-up.md`) owns the final-review Unicode offset repairs, generic likely-test compatibility restoration, and associated dead-code cleanup. Plan 98 remains the completed feature plan.
...
JavaScript follow-up amendment (2026-07-23): the harness already classified `.js`, `.jsx`, `.mjs`, and `.cjs` as canonical `javascript`, loaded `tree-sitter-javascript`, parsed them through the shared ECMAScript adapter implementation, and discovered JavaScript test conventions. The oversight was product guidance and projection: JavaScript aliased to `lang/typescript`, whose skill included TypeScript-only checks. Phase 6 adds a dedicated `lang/javascript` lens/skill and end-to-end acceptance without duplicating the proven grammar/adapter infrastructure.

Phase-6 implementation reconciliation (2026-07-24): JavaScript now projects only `lang/javascript`; `lang/typescript` is restricted to TypeScript/TSX. Structural tests cover `.js`/`.jsx`/`.mjs`/`.cjs`, callable/value/class-expression symbols, anonymous defaults, private/callable fields, JSX, ESM/re-export/CommonJS imports, bounded signatures, enclosing/changed symbols, deterministic likely tests including curried `.each`, packets, and Stage-7/9 isolation. The packed consumer loads eight skills while retaining seven grammars, and the public fake suite contains eight passing cases. A real non-posting JavaScript owner smoke passed against `ff971a1dae48d4a84a4691d9a25e13f5c117feaf..864019b3ff3baaeb96061e576eaccc1bde410b49`, detecting the floating persistence promise with exact `saveRecord` and `src/save-record.test.js::rejects when persistence fails` context while rejecting the marker-free safe control. This atomic phase commit is the boundary revision. No push, merge, tag, or publication is part of the implementation turn, so the disclosed second external Stage-5/registry/cache boundary begins only when this revision is first made externally visible.

## Problem

At plan creation, codegenie reviewed any diff but only Go and a combined TypeScript/JavaScript path received first-class syntax context and language guidance. Phases 0-5 added Rust, Python, and Solidity support. Phase 6 closes the remaining oversight: canonical JavaScript no longer projects through `lang/typescript`, and dedicated JavaScript fixtures, skill acceptance, packaging, fake transport, real owner smoke, and language-lens isolation are now present.
