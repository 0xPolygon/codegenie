...
Status: COMPLETE (harness + relay case; Phase-2 portable fixture deferred per stop condition)
Completed: 2026-07-02. Implementation notes: `repeat` runs land under `logs/<run>/repeats/<k>/` with per-execution telemetry + `score.json`, aggregated into the parent `info.json` (`repeats` block) and `eval-aggregate.json`; the determinism guard is schema-level (`repeat > 1` requires explicit `review.cache: false`, incompatible with artifact-backed cases) plus a runtime check against `--cache` overrides. Thresholds gate only when present (measure-first posture); ungated expectations report `measured only` and never fail. NOTE outcome reads `human-attention-notes.json` (both grouped and flat shapes) and marks `loss.surfacedAsNote`. The plan-83 handoff landed as `fingerprintsStable`/`distinctFingerprints` per expectation (cross-run identity assertion). Relay case authored at `codegenie-private-evals/trails-api/relay-wc/eval.yml` pinned to trails-api commit `dad51fda` (the WC call site is `relay.go:92` inside `Info()`; lineRange [82,105]; the ZG look-alike guard is in the same diff). `analyze-relay-recall.py` deletion deferred until the first real repeat-10 run reproduces the manual measurement. Phase-2 portable fixture deferred (extraction risk per stop condition; the pinned external ref is the interim).
