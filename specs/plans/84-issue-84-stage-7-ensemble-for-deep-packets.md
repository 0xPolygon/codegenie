...
Status: COMPLETE (2026-07-04) — decision record: keep, opt-in, K=2 everywhere; product default stays 1
Design amendment (2026-07-04, from run 0c4d5213/52): eligibility is no longer "every deep packet" — at most MAX_ENSEMBLED_PACKETS_PER_RUN = 8 packets ensemble per run (first in input order, remainder disclosed via `stage7_ensemble_packet_cap`). Run 52's planner assigned 14 deep packets (vs the usual 3-5), and unbounded deep×K multiplied into the wave era's first budget soft-stop breach. Running tally (runs 32/49/51/52/53): pass-2-only rescues in 32, 49, 51 (incl. run 32's expectation match by a pass-3 candidate); both-pass emissions common on productive packets ([1,1] in 51/52/53 incl. the escalated erc20 packet's match in 53); the escalated erc20 packet also went [0,0] in 52 — correctly-aimed redundancy remains probabilistic. K settings under measurement: 3 (49f4645b), 2 (0c4d5213).
