...
Post-landing observation (2026-07-02, `49f4645b` runs 29 vs 30): the same underlying bug published via different lanes anchors in different *files* (direct candidate → `hyperlane.go:220`; promotion → fabricated `process_quote.go:18`) and therefore fingerprints differently — this is inherent (they are genuinely different location claims), not a defect in the scheme. The repeat harness's `fingerprintsStable` metric quantifies it; Plan 81's anchor fixes reduce it at the source.
