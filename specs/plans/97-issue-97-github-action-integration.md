...
Status: IMPLEMENTED (2026-07-22) — dogfood: PR lane validated live 2026-07-23 (first real review posted end-to-end: status comment → checklist → report with a coherent human-attention question); comment lane ("codegenie review" comment) still to be exercised before flipping COMPLETE
...
Amendment 4 (2026-07-22, owner decision — simplicity over the preflight pattern): the two-job preflight workflows are replaced by single-job templates with workflow-level `concurrency` and a uniform `cancel-in-progress: true` (newest event wins; a push supersedes the now-stale review). Authorization still lives ONLY in the binary — the YAML contains zero gating logic, so the Amendment-3 drift/blocked-trigger problems cannot recur. Accepted residual (owner-approved): on the comment lane, any comment on a PR supersedes that PR's in-flight run before codegenie decides it is a skip — a per-PR nuisance, not privilege escalation (posting/review authorization is unaffected). Documented escape hatches for chatty repos: `cancel-in-progress: false` on the comment lane (re-triggers queue), or the retained `--preflight-only` mode (binary + action input + outputs kept and tested) in a separate ungrouped job for the strictest setup. Workflow contract tests now pin the single-job shape, binary-only authorization, and the uniform cancellation policy.
...
Template workflows are single-job (Amendment 4): workflow-level `concurrency` (`codegenie-review-pr-<n>`, per-PR groups) with a uniform `cancel-in-progress: true` — newest event wins, so a force-push cancels the stale run and the replacement reclaims the sticky comment. The YAML contains no authorization logic whatsoever; the binary's gate (exact phrase, live write-permission check) is the single source of truth, so custom phrases, associations, and `allowed-users` cannot drift or be blocked by a YAML approximation. Accepted residual: a drive-by comment supersedes that same PR's in-flight run before the binary decides it is a skip (nuisance-scope, per-PR, no authorization impact). Escape hatches, documented in the templates and README: `cancel-in-progress: false` on the comment lane (re-triggers queue; pending-run displacement is the only residual there), or the retained `codegenie github-action --preflight-only true` mode in a separate job with no concurrency group — the airtight two-job pattern for orgs that want it. The preflight capability (flag, action input, `should-run`/`pr-number` outputs) stays implemented and tested even though the default templates no longer use it.
