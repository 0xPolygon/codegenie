...
Implementation notes: `src/github-action/` (entrypoint, event-gate, status-comment, issue-comments, render, marker) + `action.yml` + `examples/workflows/`; harness touches held to the two permitted seams (`main.ts` dispatch, `loadViewerLogin` fallback via `CODEGENIE_GITHUB_LOGIN`). No new review flags — inline posting rides `--post-github-comments` in the synthesized argv. Trigger lanes, write-gate with live permission re-check, fork skip, throttled status-comment edits, terminal report with 65,536-char cap + step-summary/artifact fallbacks, and module-owned `github-action.json` telemetry all landed per Design; 23 new tests in `tests/github-action.test.ts`.
