...
Status: PHASE 1 IMPLEMENTED (2026-07-02) — eval validation pending; Phase 2 NOT shipped per stop conditions
Implementation notes (2026-07-02): Tier 1 (`inferAnchorFromChangedCode`) + Tier 2 (`representativeAnchorFromPacket`) in `pipeline-utils.ts`; Tier 1 in `stampFinding` (anchorSource `backfill_changed_code`, events `anchor_inferred`/`anchor_inference_failed`, `candidate_anchor_summary_only` no longer emitted when Tier 1 succeeds); Tier 2 in the verifier `preGateAnchor` (also after invalid-anchor strip; event `anchor_representative`); `anchorSource`/`modelAnchorSubmitted` on `CandidateFinding` and in gate facts (`modelAnchorSubmitted`/`modelAnchorValid`/`validAnchorPresent`); verdict `revisedAnchor` → `verifier_revised`. Two hazards found and closed during implementation: (a) representative anchors made distinct same-packet findings cluster as duplicates — `locationClusterKey`/`candidateScopesOverlap` now ignore `backfill_packet_representative` anchors (identity never trusts gate-only placement); (b) `revisedFinding` retained the original anchor while dropping `anchorSource`, letting a representative anchor launder through a wording revision — provenance now survives revision. Composition withholds representative anchors (`representative_anchor_withheld`) before grouping/fingerprinting, so identity and publication both stay clean. Stage 8 parity intentionally skipped (no clean source-packet plumbing). Validation still owed: the plan's N≥10 repeat protocol on `0c4d5213` (owner-run; harness ready).
