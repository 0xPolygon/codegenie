...
- `public_fixture_transport`: the public suite currently contains five fake-provider cases for core, tests, Go, TypeScript, and Rust. The Rust case has one marker-bearing positive file and one marker-free negative-control file. These assertions prove packet review, verification, composition, and anchoring transport only; structural Rust tests own parser, symbol, likely-test, lens, and skill correctness.
