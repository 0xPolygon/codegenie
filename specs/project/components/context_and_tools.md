...
Rust declarations map as follows: top-level `function_item` → `function`; `struct_item`/`union_item`/`enum_item`/`type_item` → `type`; `trait_item` → `interface`; `mod_item` → `container`; `const_item`/`static_item` → `value`; `macro_definition` → `other`. Functions/signatures directly under a trait or impl are `method`; associated types/constants retain `type`/`value`. `impl_item` is ownership context, never a standalone symbol. Trait owners use the trait name; impl owners normalize a resolvable target such as `Payment<T>` to `Payment`. The contextual signature retains the bounded impl header, so methods with the same owner/name from different trait impls remain distinguishable.
...
Handoff contract: the Stage 6 packet builder renders `buildPacketContext`'s outputs — the context metadata, the file outline, and the likely-tests list — plus its separately read enclosing-symbol source into `ReviewPacket.contextText` within `maxContextChars`, per `architecture.md`. The builder owns final sizing: ordinary symbol source starts compact, while important single-symbol packets can receive adaptive source context within the packet budget. Stage 6 may also use exact `findSymbolMentions` and `readSymbol` calls under harness context to build the lean changed-hunk relationship graph and attach bounded `relatedChangedContext`; those calls are deterministic context assembly, not broad reviewer exploration. When no parse is available, the context degrades to `path` plus a regex-scanned fallback outline and a `degradation` note; the packet still ships.
