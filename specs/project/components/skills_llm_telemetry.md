...
Every new language check must name an observable failure, a reachability/materiality and severity rule, a concrete unsafe example, a safe counterexample, and a mitigation. `BUNDLED_SKILL_WHY_LEDGER` carries evidence-bearing entries for `lang/javascript`, `lang/rust`, `lang/python`, and `lang/solidity`. Every matrix field is independently asserted per check. The full JavaScript, Rust, Python, and Solidity Stage-7 projections must each fit the 4000-character per-skill cap without truncation; compatible packet projections must also remain within the 12000-character total cap.
