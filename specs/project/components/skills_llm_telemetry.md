...
Plan 115 extends each composition entry with `sections?: Array<{ kind: "impact" | "verification" | "fix" | "test"; text: string; sourceRefs: string[] }>` and `evidenceRefs?: string[]`. New prompts require both; optional schema fields preserve legacy compatibility, whose bodies use deterministic fallback. References account for every immutable source contribution exactly once. The harness renders evidence and verification text from sources, while impact/fix/test prose may be composed; attribution does not establish semantic equivalence. `composition-sources.json` persists source components, proposals, and dispositions.

Schema recovery snapshots complete provenance-valid submissions, checks original array identities/order and protected fields before accepting or caching a repair, and retains bounded unresolved obligations across the same worker's retry. Schema-defined content remains protected; unexpected keys at explicitly closed schema objects are discarded regardless of content (Plan 116 superseding policy after run 81). Partial-prefix salvage still fails. The existing single model-repair allowance and deadlines still apply. Recovery events distinguish opened obligations, rejected preservation checks, and terminal resolution by deterministic correction/model repair/worker restart.
...
- String fields carry `maxLength` guards (titles 200, bodies 10000, evidence 4000) so a runaway model cannot bloat artifacts; over-length submissions are schema-invalid and go through the repair path. Stage 9's verifier reason has a model-facing 2,000-character target and a 4,000-character hard maximum; accepted text inside that buffer is preserved unchanged and target overflow is counted.
...
`PiRunner.runStructured` owns the agent loop. Per `architecture.md`, the agent loop is implemented inside the pi-runner, behind `LlmRunner.runStructured`: pi-ai's own `agentLoop` is not used. The production adapter consumes `Models.stream()` / `streamSimple()` while preserving the runner's single `Promise<PiAssistantMessage>` seam, and the runner then drives final provenance + `validateToolCall` per step. The pipeline's worker runner schedules workers and supplies the budget through `LlmStructuredRequest`; it does not run the loop itself (`components/review_pipeline.md`).
...
3. Response handling, in order:
   - Submit-tool call present: count it under the existing stage discipline. A trusted call proceeds through pi-ai `validateToolCall` and the optional pure semantic validator. An untrusted/provenance-less call cannot be executed or normalized: its entire assistant response is discarded, the independently constructed prior conversation is retained, and the same one-repair scheduler appends bounded state guidance for a clean forced-submit re-execution. Valid schema and semantics resolve `runStructured`; any other tool calls in the same response are ignored with a telemetry note (`submit_with_extra_tools`).
...
Plan 116's superseding policy after run 81 applies copy-only cleanup to complete trusted submit arguments before validation and cache acceptance: remove every unexpected key at explicitly closed object schemas, regardless of value. Preserve schema-defined optional fields and open-object entries; do not guess ambiguous schema branches. The exact valid verifier risk alias is handled before deletion; a conflicting unexpected alias cannot override an existing canonical field. Cache entries and debug traces retain raw provider messages and reapply cleanup on replay. `submit_shape_correction_accepted` / `submit_shape_correction_rejected` audit removed paths (`unknown_property`) or the exact alias, separately from model repair. Full schema, semantic, and schema-defined content-preservation checks still gate acceptance. Missing required or invalid fields receive the existing bounded repair with precise diagnostics, `removedUnexpectedFields` original values, and the complete original payload fenced as untrusted data. Unexpected fields are not preservation obligations; there is no fuzzy-rename checker. Known field values, array members/order, and decisions remain protected. Runner message/cache version: `pi-runner-loop-v16`. Previously absent schema-defined required or optional fields may be added during repair when their values validate; existing valid values and array contents remain protected. Stage 7 rejects contradictory `reviewStatus`/`findings` combinations before acceptance or cache writes (`review_status_findings_mismatch`). This fixes run 82, where adding optional `reviewStatus: "findings"` alongside the missing required arrays was incorrectly rejected.


Run 84 recovery follow-up: before validation, complete JSON strings at unambiguous object/array schema positions are decoded. A misplaced sibling envelope is reconstructed only when prepending the exact parent property produces one complete JSON object, with no duplicate keys (including escaped spellings), conflicting outer fields, or competing reconstructions. No prefix/suffix extraction, inferred values, or truncated containers are accepted. Unknown-key cleanup then runs normally. Raw traces/cache messages remain unchanged; edits use `complete_json_string` and `misplaced_sibling_envelope`. Repair diagnostics and the preserved inventory refer to the reconstructed draft, so actual missing fields remain visible.

An unreadable structured string emits `recovery_unusable_submission` and uses the existing single schema-repair allowance to request a fresh complete submission from retained source evidence. This does not open an impossible preservation obligation against opaque text, add attempts, extend deadlines, or erase earlier readable obligations. Full schema, stage semantics, and final-argument provenance remain required. Repeated failures terminate normally; existing worker retry policy remains unchanged. Because reconstruction fidelity cannot be demonstrated for that abandoned draft, eval `recoveryFidelity` is `unknown` (or `unresolved` if readable obligations remain), never falsely `preserved`, even when the new review completes. Opt-in `recoveryFidelity: preserved` therefore still fails in that case.


Run 85 repair merging supersedes full-payload reproduction for readable field-level failures. Codegenie sends precise missing/invalid-field diagnostics plus a retained draft. The repair tool accepts nested partial objects, a full object, or explicitly enumerated literal field-path keys. Input fields are optional because retained values need not be repeated; descriptions distinguish fields required in the final result from genuinely optional fields. Supplied values win over older overlapping values. Omitted object properties remain retained. Object-array updates merge at original indices and retain omitted items; they cannot silently delete findings/evidence. Primitive arrays supplied as field values replace their previous values. Arbitrary model-selected paths are not executable; unknown keys are removed, ambiguous nested/flat representations are rejected, and container-level failures that would require deleting data retain the existing bounded failure path.

The assembled object must pass the original full schema, stage semantics, and trusted final-argument checks before acceptance or caching. The existing one-repair allowance, low reasoning policy, and 180-second cap are unchanged. Optional omissions are no longer preservation failures for full submissions either. Telemetry records `field_repair_scheduled`, `field_repair_accepted`, and `field_repair_rejected`. When a repair revises existing content, `recovery_content_revised` records paths and fidelity is `unknown`, not falsely byte-preserved. Raw cached responses remain raw updates; replay re-merges them against the retained draft and repeats full validation. Runner message/cache version: `pi-runner-loop-v16`.


Run 86 recovery retains a working draft across worker attempts and well-typed incomplete repair patches. Later readable submissions merge into that draft before diagnostics and the next bounded repair are prepared; opaque payloads and invalid patches cannot replace it. Omitted fields and object-array entries stay retained. Updates emit `recovery_draft_updated` with remaining issues and item counts; changed existing values also emit `recovery_content_revised`. Retained progress remains advisory until full schema and semantic validation succeed. Repair schemas relax existing objects at fixed indices, but require complete required fields for newly created objects and appended array entries. The recovery inventory size cap and retry limits are unchanged.


Run 87 report-quality follow-up: Stage 10 generates exact source-ID enums and requires attributed sections/evidence references for live model submissions. Semantic validation checks group membership, source kinds, unique use, and complete finding/component accounting before acceptance and cache writes. Invalid attribution uses the existing bounded repair, with the exact diagnostic and original source inventory retained. Reasoning policies and deadlines are unchanged. Source-based rendering groups contributions by section, deduplicates identical evidence, and places additional verbatim caveats/evidence in expandable details; it does not infer semantic equivalence or delete distinct records. Existing artifacts retain complete source components and composition proposals.

Stage 9 live submissions require proofAssessment (established/refuted/unresolved, concrete evidence, and unresolved assumptions marked essential or secondary). The public schema retains optionality for historical typed artifacts; live requests require the field. Publishable verdicts must have established proof, required evidence, and no essential unresolved assumption. Unresolved hypotheses are retained as human-attention concerns with source paths, while established defects may retain secondary uncertainty. This is a structured model assessment, not machine proof of semantic truth. The gate also applies before cache acceptance; defensive normalization protects custom adapters.

Eval metrics now expose compositionQuality separately (non-degraded/degraded/unknown). Setting expect.compositionQuality: non-degraded fails degraded or missing composition evidence without conflating it with coverage completeness or recovery fidelity. No private eval configuration is changed.

Run 88 recovery follow-up: an unusable later response does not discard a readable retained draft's update policy. A trusted repair merges supplied revisions and retains omitted items, then revalidates the complete submission; changed paths remain audited as content revisions. Verifier repair explicitly declares alternative top-level encodings (`finalFinding` / `findingUpdates`): supplying exactly one replaces its retained sibling, while supplying neither retains the prior representation. Both supplied representations must agree on every compact-update field; matching redundancy collapses locally after validation, and conflicting fields appear in repair diagnostics. This does not permit arbitrary field deletion or add repair attempts.

Bounded repeated repair (supersedes the progress-gated policy): a worker may make up to three schema repair calls after its original submission, even if earlier repairs make no progress. Missing fields, invalid values, malformed submissions and semantic errors all remain eligible within the allowance. Every repair retains the readable draft, requests the current corrections, and must pass complete schema and semantic validation before acceptance or caching. All three share 180 seconds of wall time, including queueing, and obey cancellation. On exhaustion, the existing worker retry policy applies. Composition retains its two primary-call retry limit and six-minute overall deadline; its schema repairs use the separate three-repair allowance and shared repair deadline. The event `schema_repair_retry_scheduled` records attempt, maximum attempts, remaining paths/time, and obligation ID. Location clarification retains its separate single-call policy.
