...
- Before Stage 7 projection, skills are defensively filtered to language-neutral skills or skills whose `languages` contains the packet's canonical language. Stage 9 reapplies the same rule to the recorded producing skills using the candidate packet/diff language, preventing JavaScript/TypeScript/Rust/Python/Solidity content from entering one another's verifier prompts or Go prompts. A targeted Stage-8 task may deliberately span languages; it records the exact skills its task prompt actually projected, and Stage 9 performs the candidate-specific filter.
...
- `producedBy` is stamped deterministically by the runner: `{ kind: "packet", stage: 7, packetId, lensId, skillIds, workerId }`. `lensId` is the packet's primary (first) lens and remains attribution only. `skillIds` is the ordered, deduplicated set of non-omitted skills that contributed characters to that specific producing pass's built prompt. It is derived from `BuiltPrompt.projection`, never from current primary-lens membership. The model claims neither field (`components/skills_llm_telemetry.md`).
- Anchor validation uses the entire PR diff: side-appropriate path, changed line, and matching hunk id. Discovery packet attribution and coverage remain independent of finding location. Invalid anchors are stripped; exact quoted changed lines may recover a unique location across the diff, normalizing only indentation, displayed line-number columns, and diff prefixes established by numbered lines, explicit diff fences, or hunk headers. Bare leading unary plus/minus operators are never stripped. Substrings, ambiguous matches, and matches across files/sides/hunks cannot establish placement.
- After packet outcomes are retained, unresolved locations receive at most one optional clarification per candidate, with existing concurrency/global budget checks, no repository tools, lowest supported reasoning, a 180-second deadline, and no provider retry or schema-repair loop. The response can only supply validated coordinates or report no suitable location; candidate evidence and discovery attribution are immutable. Failure leaves the finding summary-only, not an incomplete investigation. Run cancellation and fatal provider errors still propagate. Telemetry records `candidate_location_clarification` outcomes and candidates retain `locationResolution`.
- Verification receives the actual anchored hunk plus separately labeled discovery context. It does not synthesize representative coordinates for anchorless candidates. Evidence-backed low-confidence candidates may enter the bounded evidence-resolution lane without an inline location; coordinate validity never establishes claim validity. Existing composition, threshold, and duplicate policies remain in force.
...
- Each surviving `followUpHint` and each `uncertainty` receives required pipeline-stamped `projectedSkillIds` from its specific producing pass, after model-schema validation. Ensemble/adaptive pooling and question deduplication preserve the selected item's list without equality requirements, unioning, or a packet-level approximation; an uncertainty-promotion candidate inherits only its selected source item's list. Telemetry events (`event: "follow_up_hint"` / `"uncertainty"`) persist that ordered list with the existing pointer-rich data — the hint-event reader contract consumed by `components/evals.md`.
...
- Candidate findings produced by Stage 8 are stamped with the exact ordered, non-omitted ids from the Stage-8 prompt projection, appended to the candidate pool, and pass through normal Stage 9 verification. Resolved hints do not become candidates and need no provenance carrier.
...
Verifier dispatch: one candidate per call through the worker runner (stage 9, `review.perPassTimeoutMs`, bounded by `review.concurrency` and `llm.maxConcurrentCalls`); the verifier is an independent call with fresh context, using the run's single resolved model. Verifier `ToolBudget` default: `{ maxToolCalls: 6, maxInvestigationRounds: 2, maxResultChars: 12000 }` (implementation constant). Guidance is resolved only through `skillsById(candidate.producedBy.skillIds)`, then defensively filtered for the candidate packet's canonical language. Unknown and language-incompatible ids are dropped and reported; `lensId` is never a selection input, and an empty authoritative list produces no skill header or guidance label. The verifier receives the candidate, originating packet context, relevant hunks, cited evidence, those exact False Positives/Safe Patterns projections, and the read-only tool suite — for proving, narrowing, or rejecting the specific claim only; it must not search for new issues, and verifier-introduced unrelated findings are dropped in validation with telemetry.

Verdict handling (`VerificationVerdict` per candidate):

- `keep` → a bare keep means confidence, severity, evidence, wording, and placement are publishable unchanged. Historical/provider keep verdicts carrying `finalFinding` or `revisedAnchor` are canonicalized to `revise`, with `verification_keep_payload_canonicalized` telemetry naming the payload kinds.
- `reject` → recorded with reason; excluded.
- `revise` → the provider-safe flat submit schema keeps both compatibility payloads optional, then runtime semantic validation requires a non-empty structured change: either `finalFinding` or `revisedAnchor`. `finalFinding` must preserve the candidate id (contract); a `revisedAnchor` is accepted only if it validates against a changed diff line, otherwise the original validated anchor is preserved; a real-but-unanchorable issue may revise through a complete `finalFinding` and proceed as summary-only. Severity/confidence/wording/fix/test narrowing is accepted as submitted; lineage is preserved. An empty revise is recorded as incomplete with `verification_semantic_invalid`, never as a verifier rejection.
...
- `verify_revision_payload_contract`: the provider-facing verdict schema has one object root; runtime semantics persist empty revise as incomplete, legacy keep-with-payload canonicalizes to revise, and a bare keep remains unchanged.
- `verify_empty_primary_submit_fails_closed`: a run-60-shaped evidence-backed candidate whose primary verdict arguments are `{}` gets a bounded evidence-bearing stateless repair prompt, distinct empty-submit telemetry, and an incomplete outcome even when repair returns a schema-valid reject.
- `promotion_related_signal_provenance`: run-57-shaped exact-input/output selection retains byte-identical selected ids/order, while related unselected output framings attach only to the output candidate, cap at eight, leave evidence/confidence unchanged, and unmatched or overflow signals remain lane-limited.
...
- `compose_verified_low_confidence_delta_hatch`: evidence-backed complete keep/revise verdicts may publish concrete low-confidence behavior deltas; trusted anchors retain their mode, withheld representative anchors force summary-only, verdict-quality failures suppress, representative-local merged-member qualification cannot leak, and the ordinary report cap still applies.
