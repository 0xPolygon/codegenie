...
Codegenie's built-in model overrides prefer the `deepseek` upstream for OpenRouter models with IDs starting with `deepseek/` (`order: ["deepseek"]`), and allow another upstream when that one cannot serve the request. A hard `only` pin with `allow_fallbacks: false` failed every call when the account excluded that upstream or it returned 4xx, and the review finished with zero tokens. For this model family, submit calls expose only the submit tool and use `tool_choice: "auto"`, to accommodate the named forced-tool rejection observed on DeepSeek V4.1 Flash. Returned submissions still undergo strict validation, and missing submissions have bounded retries. These [OpenRouter routing preferences](https://openrouter.ai/docs/guides/routing/provider-selection) apply to every stage, including repairs; Codegenie's stage-specific reasoning levels still apply. Routing is included in debug request traces and local model-call cache keys. The overrides live in `src/provider/models-override.ts`.
